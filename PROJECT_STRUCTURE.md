...
这是一款 Flutter + Android 原生低延迟节拍引擎的节拍器应用。Flutter 负责三页 UI、WebView、设置抽屉、配置保存、倒计时和状态展示；Android 原生层负责 SoundPool 播放、前台服务、音频焦点、WakeLock、振动、TTS 和节拍事件回传。
...
- `吉他社`：`GuitarSocietyPage`，加载 `https://www.jitashe.org/` 的 WebView，右下角有共享播放状态的 Start/Stop 悬浮按钮。
- `节拍器`：默认首页，包含顶部功能按钮、轻重拍编辑条、BPM 圆盘、Start/Stop、统计面板。
- `设置`：`SettingsPage`，支持输入名称保存当前配置、查看、恢复和删除已保存配置。

播放状态集中在 `_MetronomeMainPageState`，首页 Start 按钮、WebView 悬浮按钮、倒计时自动停止都调用同一套 `_setPlayback` / `_togglePlayback` 逻辑。
...
- `TransportPanel`：Start/Stop 控制区。第二轮已删除底部 `BEAT` 计数卡片，避免被导航栏裁剪。
- `PracticeStatsPanel`：今日练习、BARS、ENGINE 状态和最近练习记录。
- `TimeSignatureSheet`：拍号抽屉，双滚轮选择拍数 `1~16` 和时值 `1/2/4/8/16/32`，并提供快捷拍型。
- `SoundPresetSheet`：音色 UI 入口。第二轮暂不继续开发真实音源逻辑。
- `TunerSheet`：调音器占位。未接入真实麦克风识别前只显示“调音器开发中”。
- `TimerSheet`：定时抽屉，Apply 后启动倒计时，首页定时按钮显示剩余时间，归零后停止节拍器。
...
- `MainActivity.kt`：注册 MethodChannel / EventChannel，接收 Flutter 指令并转发给服务或状态缓存。
...
- `lib/main.dart` 已接近 4000 行。下一轮大改建议优先拆分为 `pages/`、`widgets/`、`models/`、`services/`，拆分前后都要保持测试通过。
